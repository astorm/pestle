...
* Generates a Magento 2.1 ui grid listing and support classes.
*
* @command magento2:search:requirejs
* @argument search_terms Which Seach Terms? []
*/
function pestle_cli($argv)
{    
    $base = \Pulsestorm\Magento2\Cli\Library\getBaseMagentoDir();
    $cmd  = 'find ' . escapeshellarg($base) . '/vendor/magento ' . 
    '-name requirejs-config.js | xargs grep -i -r '
    . escapeshellarg($argv['search_terms']);
    passthru($cmd);
}
}
...
namespace Pulsestorm\Wordpress\Uurls{
use function Pulsestorm\Pestle\Importer\pestle_import;
